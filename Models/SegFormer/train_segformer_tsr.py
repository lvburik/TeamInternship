...
def TSR(video):
    ...
    second_derivatives = np.empty((n_pixels, d), dtype=np.float32)
    ...
    for i in range(n_pixels):
        ...
        #second_derivatives[i] = second_derivative
...
for video in video_files:
    ...
    #second=second.T
    #second=second.reshape(-1,480,640)
...
print(f"Number of frames is {len(all_first_dev_frames)}")
...
unloaded_dataset=dataset(all_first_dev_frames, video_one)
